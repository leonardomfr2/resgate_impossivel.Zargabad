...
if (count _templateOld == 0) exitWith {
    0 = ["Zen_UpdateFireSupport", "Given template does not exist", _this] call Zen_PrintError;
    call Zen_StackPrint;
    call Zen_StackRemove;
};

if (_roundType isEqualTo -1) then {_roundType = _templateOld select 0};
if (_roundsPerSalvo isEqualTo -1) then {_roundsPerSalvo = _templateOld select 1};
if (_salvos isEqualTo -1) then {_salvos = _templateOld select 2};
if (_timePerRound isEqualTo -1) then {_timePerRound = _templateOld select 3};
if (_timePerSalvo isEqualTo -1) then {_timePerSalvo = _templateOld select 4};
if (_targetArea isEqualTo -1) then {_targetArea = _templateOld select 5};
if (_salvoDrift isEqualTo -1) then {_salvoDrift = _templateOld select 6};
if (_guided isEqualTo -1) then {_guided = _templateOld select 7};
