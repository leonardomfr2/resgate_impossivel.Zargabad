...
private ["_pos", "_class", "_vehicle", "_height", "_special", "_dir", "_collide"];
...
if (surfaceIsWater (getPosATL _vehicle)) then {
    if (_vehicle isKindOf "SHIP") then {
        if (_collide) then {
            0 = [_vehicle, _pos, -(getTerrainHeightASL (getPosATL _vehicle)) + _height, _dir] call Zen_TransformObject;
        } else {
            0 = [_vehicle, _vehicle, _height, _dir] call Zen_TransformObject;
        };
    } else {
        if (_vehicle isKindOf "AIR") then {
            if (_collide) then {
                ...
                if (_special == "FLY") then {
                    0 = [_vehicle, _vehicle, -(getTerrainHeightASL (getPosATL _vehicle)) + _height - 60, _dir] call Zen_TransformObject;
                } else {
                    0 = [_vehicle, _vehicle, -(getTerrainHeightASL (getPosATL _vehicle)) + _height, _dir] call Zen_TransformObject;
                };
            };
        } else {
            if (_collide) then {
    ...
    if (_collide) then {
        0 = [_vehicle, _pos, _height, _dir, (_height < 2)] call Zen_TransformObject;
    } else {
        0 = [_vehicle, _vehicle, _height, _dir, (_height < 2)] call Zen_TransformObject;
    };
};
