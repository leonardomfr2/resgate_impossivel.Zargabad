...
private ["_giver", "_receiver", "_receiverAcceptedMags", "_magName", "_giverMags", "_giveAnim"];
...
_receiverAcceptedMags = getArray (configFile >> "CfgWeapons" >> (primaryWeapon _receiver) >> "magazines");
...
if ([_giverMags, _receiverAcceptedMags] call Zen_ValuesAreNotInArray) exitWith {
    hint format ["You have no magazines that match %1's primary weapon", (name _receiver)];
    call Zen_StackRemove;
};
