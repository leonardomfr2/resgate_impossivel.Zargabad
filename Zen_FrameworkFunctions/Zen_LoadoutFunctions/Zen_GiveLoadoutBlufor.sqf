...
private ["_units", "_kit", "_giveGL", "_i", "_weapon", "_magazine", "_unit"];
...

{
    ...
    if ([(_this select 1), ""] call Zen_ValuesAreEqual) then {
        ...
        _kit = [(_this select 1)] call Zen_ArrayGetRandom;
    };
    ...
    switch (toLower ([_kit] call Zen_StringRemoveWhiteSpace)) do {
        case "recon": {
            ...
            _x addGoggles ([STD_GOGGLES] call Zen_ArrayGetRandom);
...
if (isMultiplayer && {_sendPacket}) then {
    Zen_MP_Closure_Packet = ["Zen_GiveLoadoutBlufor", _this + [false]];
